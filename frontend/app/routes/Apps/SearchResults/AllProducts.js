...
export default class AllProducts extends React.Component {
    ...
    fetchBestBuy = (input) => {
        ...
        fetch(process.env.API_URL + '/product/online?' + serialize({ kw: input, category: this.state.selected_category.id, supplier: "bestbuy", update: false }))
            ...
            .then((products) => {
                ...
                this.handleFetchFinish(input, products, timeDiff)
            })
            .catch(console.log)
    }
    ...
    //     fetch(process.env.API_URL + '/product/supplier/map/')
    //         .then(res => res.json())
    //         .then((data) => {
    //             console.log(data)
    //             let productMap = {}
    //             data.map(ps => {
    //                 if(productMap[ps.p_id]){
    //                     productMap[ps.p_id].push(ps)
    //                 }else{
    //                     productMap[ps.p_id] = [ps]
    //                 }
    //             })
    //             this.fetchDefaultProducts(productMap)
    //         })
    //         .catch(console.log)
    // }
                    ...
                    {this.state.suppliers.map((supplier, index) => {
                        return (
                            <NavItem className="d-flex px-2 mb-2">
                                {index == 0 ?
                                    <CustomInput key={index} type="radio" id={index} name="rating" label={capitalizeFirstLetter(supplier.s_name)} inline defaultChecked onChange={() => this.supplierOnClick(supplier)} /> :
                                    <CustomInput key={index} type="radio" id={index} name="rating" label={capitalizeFirstLetter(supplier.s_name)} inline onChange={() => this.supplierOnClick(supplier)} />
                                }
